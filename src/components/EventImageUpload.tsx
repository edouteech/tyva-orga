...
const EventImageUpload: React.FC<EventImageUploadProps> = ({
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      // Upload de l'image
      const updatedEvent = await eventsAPI.uploadImage(eventId, file);

      // Mettre à jour l'URL de l'image
      onImageChange(updatedEvent.main_image_url);
  ...
  const handleRemoveImage = async () => {
    ...
    try {
      await eventsAPI.removeImage(eventId);
      onImageChange(null);
    } catch (err) {
      setError(
        err instanceof Error
    ...
    <div className={`space-y-3 ${className}`}>
      ...
      <div className="space-y-3">
        ...
        <input
          ...
          aria-label="Sélectionner une image pour l'événement"
        />
